-- 집계함수 , GROUP BY
-- 고객이 주문한 도서의 총 판매액
SELECT sum(saleprice) AS [총 매출] -- 띄어쓰기를 하려면 []를 사용해서 쓰기
  FROM Orders;
...
SELECT sum(saleprice) '김연아고객 총판매액'
  FROM Orders
 WHERE custid = 2;
...
-- GROUP BY -필요조건으로 그룹핑을 해서 결과(통계)를 내기위함(집계함수 하나는 무조건 들어가야함)
...
-- Having은 등 통계 , 집합함수의 필터링조건
-- 가격이 8000원 이상인 도서를 구매한 고객에 대하여 고객별 주문도서 총수량 
-- 단, 2권 이상 구매한 경우만...
