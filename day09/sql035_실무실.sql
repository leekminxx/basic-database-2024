/* 5. 현재의 날짜 타입을 날짜 함수를 통해서 확인
    2006년 5월 20일 부터 2007년 5월20일 사이에 고용된 사원의 이름(First + Last) , 사원번호 , 고용일자
...
SELECT FIRST_NAME + ' ' + LAST_NAME + 'is a ' + JOB_ID AS 'Employee JOBS'
 ...
 /* 8. 이름. 급여, 수당 여부에 따른 연봉을 포함하여 출력, Salary + Commission / Salary Only, 연봉이 높은 순(107개행) */
...
-- 결제 함수, sum , COUNT , AVG , MAX , MIN ,...
/* 11. 사웓들의 업무별 전체 급여 평균이 10,0000$ 보다 큰 경우를 조회, 업부 , 급여평균을 출력하시오 , 단 사원(CLERK) 이 포합된 경우는 제외,
      전체 급여 내림차순으로(7개행)  */
SELECT JOB_ID
     , '$' + FORMAT(AVG(SALARY) , '#,#')
  FROM employees
...
-- CASE 연산자(프로그래밍적인)
/* 107명의 직원중 HR_REP(10%), MK_REP(12%), PR_REP(15%) , SA_ERP(18%), IT_PROG(20%)
*/
SELECT EMPLOYEE_ID
     , FIRST_NAME + ' ' + LAST_NAME as 'name'
     , JOB_ID
...
-- 부서번호 , 업무를 기준으로 그룹별로 나누고 급여합꼐와 인원수를 출력(20개행)
...
-- 사원들의 부서별 급여 기준으로 내림차순으로 정렬 , 순위를 표시하시오
SELECT EMPLOYEE_ID
     , LAST_NAME 
     , SALARY
     , DEPARTMENT_ID
