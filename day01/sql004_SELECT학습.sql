...
-- 출판사가 굿스포츠와 대한미니더가 아닌 도서를 검색하시오
SELECT *
	FROM book
...
SELECT bookname , publisher
	FROM Book
   WHERE Bookname = '축구%'; --축구라는 글자로 시작하는 

SELECT bookname , publisher
	FROM Book
   WHERE Bookname = '%축구'; -- 축구라는 글자로 끝나는 


-- 도서 이름중에 축구가 포함된 출판사를 검색하시오 
SELECT bookname , publisher
	FROM Book
...
SELECT bookname , publisher
	FROM Book
   WHERE bookname LIKE '_구%' ; -- _(무슨글자든 한글자가 들어간) 구라는 글자로 시작하는   _개수로 앞쪽에 글자수를 나타내줌 
